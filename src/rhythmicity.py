...
# Minimum evidence level below which neither lagged-coherence nor
# cycle-consistency counts as "any rhythmic evidence at all". Used as a
# strict `>` gate in both `classify_rhythmicity` and
# `compute_rhythmicity_index` so the label and the index agree at the 0.1
# boundary.
MINIMAL_EVIDENCE_THRESHOLD: float = 0.1
...
def compute_rhythmicity_index(
    ...
    Minimal-evidence gate is strict (`> MINIMAL_EVIDENCE_THRESHOLD`) to match
    the class rule in `classify_rhythmicity`, so both functions agree at the
    exact 0.1 boundary.
    """
    eps = 1e-12
    ...
    # Minimal-evidence scores keep the original CONTINUOUS scaling, but are
    # clamped to 0 when the value sits at or below the shared
    # MINIMAL_EVIDENCE_THRESHOLD. This makes compute_rhythmicity_index agree
    # with classify_rhythmicity exactly at the 0.1 boundary (strict `>` gate)
    # without collapsing the index into a step function for slightly-above
    # values that previously contributed fractionally to the weak gate.
    minimal_phase_score = (
        max(lc, curve_mean) / MINIMAL_EVIDENCE_THRESHOLD
        if max(lc, curve_mean) > MINIMAL_EVIDENCE_THRESHOLD
        else 0.0
    )
    minimal_consistency_score = (
        mean_corr / MINIMAL_EVIDENCE_THRESHOLD
        if mean_corr > MINIMAL_EVIDENCE_THRESHOLD
        else 0.0
    )
    ...
    if strong_gate_score >= 1.0:
        return float(2.0 + (strong_gate_score - 1.0))
...
def classify_rhythmicity(signal: np.ndarray, fs: float,
    ...
    minimal_phase = max(lc, curve_mean_coherence) > MINIMAL_EVIDENCE_THRESHOLD
    ...
    elif (weak_phase and mean_corr > MINIMAL_EVIDENCE_THRESHOLD) or \
